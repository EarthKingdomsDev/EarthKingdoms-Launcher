...
const msftLoginLogger = LoggerUtil.getLogger('Connexion avec un compte Microsoft')
const msftLogoutLogger = LoggerUtil.getLogger('Déconnexion d\'un compte Microsoft')
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            setOverlayContent(
                ...
                'L\'authentification a votre compte Microsoft a échoué. Veuillez réessayer plus tard.',
                'Ok'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Attention<br>C\'est votre dernier compte de connecté',
                    'Pour utiliser le launcher, un compte avec une copie de Minecraft Java Edition est requis. Vous devrez vous reconnectez après.<br><br>Êtes-vous sûr(e) de vouloir vous déconnectez ?',
                    'J\'en suis sûr(e)',
                    'Annuler'
                )
                setOverlayHandler(() => {
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                'Quelque chose c\'est mal passé',
                'La déconnexion de votre compte Microsoft a échoué. Veuillez réessayer plus tard.',
                'Ok'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Utilisateur</div>
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte Sélectionné &#10004;' : '>Compte Sélectionné'}</button>
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/FallenGloryDevelopment/FallenGlory-Launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle ${isPrerelease(data.version) ? 'Pre-release' : 'Mise à jour'} Disponible`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Télécharger depuis GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermer le launcher et exécuter le .dmg pour mettre à jour.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = 'Vous possédez la dernière version stable !'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Rechercher des Mises à jour', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Rechercher des Mises à jour...', true)
            }
        })
    }
}
