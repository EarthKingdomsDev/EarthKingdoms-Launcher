    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            if(arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLoginLogger.info('La connexion a été annulé par l\'utilisateur.')
                return
            }
        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser ce Launcher, vous devez être connecté à au moins un compte. Vous devrez vous reconnecter après.<br><br>Voulez-vous vraiment vous déconnecter ?',
                    'Oui',
                    'Annuler'
                )
                setOverlayHandler(() => {
...
function populateJavaReqDesc(server) {
    settingsJavaReqDesc.innerHTML = `Requirs Java ${server.effectiveJavaOptions.suggestedMajor} x64.`
}
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/EarthKingdomsDev/EarthKingdoms-Launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle ${isPrerelease(data.version) ? 'Pre-release' : 'Version'} Disponible !`
        ...
        settingsUpdateTitle.innerHTML = 'Vous avez actuellement la dernière version du Launcher'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Recherche une Mise à jour', false, () => {
            if(!isDev){
